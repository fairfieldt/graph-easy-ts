...
function perlScalarLocaltime(d: Date): string {
  // Match Perl's `scalar localtime()` (ctime-style) format:
  //   "Fri Dec 26 18:32:55 2025"
  const DOW = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"] as const;
  const MON = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"] as const;

  const pad2 = (n: number): string => String(n).padStart(2, "0");
  const padDay = (n: number): string => String(n).padStart(2, " ");

  const dow = DOW[d.getDay()] ?? "Sun";
  const mon = MON[d.getMonth()] ?? "Jan";
  const day = padDay(d.getDate());
...
export function renderGraphviz(graph: Graph): string {
  ...
  // Match Perl's banner format: `" at " . scalar localtime()`.
  txt += `  // Generated by Graph::Easy ${GRAPH_EASY_VERSION_FOR_BANNER} at ${perlScalarLocaltime(new Date())}\n\n`;
