...
function runTsBoxart(nodeExecutable: string, asBoxartJs: string, inputPath: string): childProcess.SpawnSyncReturns<string> {
  return childProcess.spawnSync(nodeExecutable, [asBoxartJs, inputPath], {
    encoding: "utf8",
    maxBuffer: 20 * 1024 * 1024,
  });
}

function runPerlBoxart(repoRoot: string, inputPath: string): childProcess.SpawnSyncReturns<string> {
  // Use upstream Perl as the oracle.
  const perlCwd = path.join(repoRoot, "Graph-Easy-0.76");
  return childProcess.spawnSync("perl", ["examples/as_boxart", inputPath], {
    cwd: perlCwd,
    encoding: "utf8",
    maxBuffer: 20 * 1024 * 1024,
  });
...
function main(): void {
  ...
  const asBoxartJs = path.join(repoRoot, "dist", "examples", "as_boxart.js");
  ...
  for (let i = 0; i < fixtures.length && i < limitCases; i++) {
    ...
    const perl = runPerlBoxart(repoRoot, inputPath);
    if (perl.status !== 0) {
      skip++;
    ...
    const ts = runTsBoxart(process.execPath, asBoxartJs, inputPath);
    if (ts.status !== 0) {
      fail++;
